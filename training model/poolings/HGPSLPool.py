...
from poolings.sparse_softmax import Sparsemax
...
class TwoHopNeighborhood(object):
    def __call__(self, data):
        ...
        index, value = spspmm(edge_index, value, edge_index, value, n, n, n)  # self loop怎么办
        value.fill_(0)
...
class NodeInformationScore(MessagePassing):
    ...
    def norm(edge_index, num_nodes, edge_weight, dtype=None):
        ...
        row, col = edge_index
        deg = scatter_add(edge_weight, row, dim=0, dim_size=num_nodes)  # compute Degree matrix(D)
        deg_inv_sqrt = deg.pow(-0.5) 
        deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0

        edge_index, edge_weight = add_remaining_self_loops(edge_index, edge_weight, 0, num_nodes)  # add self loop
...
class HGPSLPool(torch.nn.Module):
    def __init__(self, in_channels, ratio=0.8, sample=False, sparse=False, sl=True, lamb=1.0, negative_slop=0.2):
        ...
        self.sparse = sparse
        self.alpha=0.5
        self.encoder4 = Encoder4(in_channels)
    ...
    def forward(self, x, edge_index, edge_attr,k, batch=None):
        if batch is None:
            batch = edge_index.new_zeros(x.size(0))
        x = x.unsqueeze(-1) if x.dim() == 1 else x
        x_information_score = self.calc_information_score(x, edge_index, edge_attr)
        score_s = torch.sum(torch.abs(x_information_score), dim=1)
        score_f = self.fbtl_layer(x).squeeze()
        # score_f=self.encoder4(x).squeeze()
        # hyperparametr alpha
        score = score_s * self.alpha + score_f * (1 - self.alpha)
        # score = score_s  + score_f * self.alpha
